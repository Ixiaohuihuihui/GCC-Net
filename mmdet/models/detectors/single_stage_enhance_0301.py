...
class SingleStageTwoBranchDetector(BaseDetector):
    ...
    def extract_feat(self, img, img_retinex):
        ...
        x, y = self.backbone(img, img_retinex)
        # y = self.backbone(img_retinex)
        # if self.enhance_fusion:
        #     out1 = self.enhance_fusion(x[0], y[0], None, None)
        #     out2 = self.enhance_fusion(x[1], y[1], None, None)
        #     out3 = self.enhance_fusion(x[2], y[2], None, None)
        #     out4 = self.enhance_fusion(x[3], y[3], None, None)
        #     outs = tuple((out1, out2, out3, out4))
        #     if self.with_neck:
        #         outs = self.neck(outs)
        #     return outs
        # else:
    ...
    def forward_train(self,
        ...
        x, y = self.extract_feat(img, img_retinex)
        edge_map = DeepSobel(y[1])
        edge_att = self.edge_attention(edge_map)
        # heatmap =cv2.applyColorMap(np.uint8(255 * edge_map.detach().cpu().numpy()), cv2.COLORMAP_JET)
        # cv2.imwrite("demo/temp_edge/heatmap.jpg", heatmap)
        if self.enhance_fusion:
            out1 = self.enhance_fusion(x[0], y[0], self.upsample_2(edge_att), None)
            out2 = self.enhance_fusion(x[1], y[1], edge_att, None)
            out3 = self.enhance_fusion(x[2], y[2], self.downSample(edge_att), None)
            out4 = self.enhance_fusion(x[3], y[3], self.downSample(self.downSample(edge_att)), None)
            out5 = self.enhance_fusion(x[4], y[4], self.downSample(self.downSample(self.downSample(edge_att))), None)
    ...
    def simple_test(self, img, img_metas, img_retinex, coffient, rescale=False):
        ...
        edge_map = DeepSobel(feat_y[1])
        edge_att = self.edge_attention(edge_map)
        if self.enhance_fusion:
            out1 = self.enhance_fusion(feat_x[0], feat_y[0], self.upsample_2(edge_att), coffient)
            out2 = self.enhance_fusion(feat_x[1], feat_y[1], edge_att, coffient)
            out3 = self.enhance_fusion(feat_x[2], feat_y[2], self.downSample(edge_att), coffient)
            out4 = self.enhance_fusion(feat_x[3], feat_y[3], self.downSample(self.downSample(edge_att)), coffient)
            out5 = self.enhance_fusion(feat_x[4], feat_y[4],
                                       self.downSample(self.downSample(self.downSample(edge_att))), coffient)
            tmp = tuple((out1, out2, out3, out4, out5))
