...
class Bottleneck(BaseModule):
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            # identity_y = y
            # out_y = self.conv1(y)
            # out_y = self.norm1(out_y)
            # out_y = self.relu(out_y)

            # if self.with_plugins:
            #     out_x = self.forward_plugin(out_x, self.after_conv1_plugin_names)
            #     out_x = self.forward_plugin(out_x, self.after_conv1_plugin_names)
            #
            ...
            # # out_y = self.norm2(out_y)
            # # out_y = self.relu(out_y)
            #
            # if self.with_plugins:
            #     out_x = self.forward_plugin(out_x, self.after_conv2_plugin_names)
            #     out_y = self.forward_plugin(out_y, self.after_conv2_plugin_names)
            #
            # out_x = self.conv3(out_x)
            # out_y = self.norm3(out_y)
            #
            # if self.with_plugins:
            #     out_x = self.forward_plugin(out_x, self.after_conv3_plugin_names)
            #     out_y = self.forward_plugin(out_y, self.after_conv3_plugin_names)
            #
            # if self.downsample is not None:
            #     identity_x = self.downsample(x)
            #     identity_y = self.downsample(y)
            #
            # out_x += identity_x
            # out_y += identity_y
            #
...
class ResNet_TwoBranch(BaseModule):
    """ResNet backbone.
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(ResNet_TwoBranch, self).__init__(init_cfg)
    ...
    def forward(self, x, y):
        ...
        y1 = getattr(self, 'layer1')(y)
        tmp = torch.cat((x1, y1))
        x2 = getattr(self, 'layer2')(tmp)
        # y2 = getattr(self, 'layer2')(y1)
        # y3 = getattr(self, 'layer3')(y2)
        # y4 = getattr(self, 'layer4')(y3)

        # if self.enhance_fusion:
        #     out1 = self.enhance_fusion(outs_x[0], outs_y[0], None, None)
        #     out2 = self.enhance_fusion(outs_x[1], outs_y[1], None, None)
        #     out3 = self.enhance_fusion(outs_x[2], outs_y[2], None, None)
        #     out4 = self.enhance_fusion(outs_x[3], outs_y[3], None, None)
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(ResNet_TwoBranch, self).train(mode)
        self._freeze_stages()
