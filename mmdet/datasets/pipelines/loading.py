...
class LoadImageFromFile:
    ...
    def __call__(self, results):
        ...
        if results['img_prefix'] is not None:
            ...
            # filename_res = osp.join(results['img_prefix'] + 'output_images/',
            #                     results['img_info']['filename'])
        else:
            filename = results['img_info']['filename']

        img_bytes = self.file_client.get(filename)
        # img_bytes_res = self.file_client.get(filename_res)
        img = mmcv.imfrombytes(
            img_bytes, flag=self.color_type, channel_order=self.channel_order)
        # img_res = mmcv.imfrombytes(
        #     img_bytes_res, flag=self.color_type, channel_order=self.channel_order)
        ...
        # results['img_retinex'] = img_res
        # results['img_fields'] = ['img', 'img_retinex']
