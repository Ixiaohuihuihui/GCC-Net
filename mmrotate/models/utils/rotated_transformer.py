...
class RotatedDeformableDetrTransformer(Transformer):
    ...
    def forward(self,
        ...
        if self.as_two_stage:
            ...
            # -----------------------------------
            # from mmrotate.core.visualization.image import show_topk_proposals
            # import mmcv
            # cd = topk_coords_unact.clone()
            # cd[..., :4] = cd[..., :4].sigmoid()
            # cd[..., :4] = cd[..., :4] * 1024
            # img = mmcv.imread('/data2/dailh/split_1024_dota1_0/trainval/images/P1388__1024__0___0.png').astype(np.uint8)
            # colors = np.ones([len(cd[0])])
            # colors = colors.astype(int)
            # out_image = show_topk_proposals(
            #     img, cd[0].detach().cpu().numpy(), colors, out_file='e.png', show=False)
        else:
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 4:
                ...
                # reference_points_xywh = reference_points[..., :4]
                reference_points_input = reference_points[:, :, None] * \
                                         torch.cat([valid_ratios, valid_ratios, valid_ratios, valid_ratios], -1)[:, None]
            else:
