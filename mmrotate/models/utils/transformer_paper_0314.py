...
# from mmdet.core import RotBox2Polys_torch_ref, distance2rbbox, polygonToRotRectangle_torch
    ...
    def forward(self,
        ...
        if self.as_two_stage:
            ...
            enc_outputs_class = cls_branches[self.decoder.num_layers](
                output_memory)
            enc_outputs_coord_unact_angle = reg_branches[
                                               self.decoder.num_layers](output_memory)
            # 转成8点坐标,
            # enc_outputs_coord_unact_rrot = RotBox2Polys_torch_ref(enc_outputs_coord_unact_angle)
            # 加上点的偏移

            # enc_outputs_coord_unact_proposals = distance2rbbox(output_proposals, enc_outputs_coord_unact_rrot)
            # cc = []
            # from mmdet.core import polygonToRotRectangle_batch
            # for i in range(bs):
            #     aa = polygonToRotRectangle_torch(enc_outputs_coord_unact_proposals[i])
            #     cc.append(aa)

            # enc_outputs_coord_unact_angle[..., :4] = \
            #     reg_branches[
            #         self.decoder.num_layers](output_memory)[..., :4] + output_proposals
            # enc_outputs_coord_unact_angle = torch.stack(cc)
            enc_outputs_coord_unact_angle[..., :4] = \
                reg_branches[
                    self.decoder.num_layers](output_memory)[..., :4] + output_proposals
